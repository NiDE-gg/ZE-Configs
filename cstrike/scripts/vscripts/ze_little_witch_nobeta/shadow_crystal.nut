...
function ItemPickUp(activator) {
	ClientPrint(activator, 3, format("\x0768F9C4[Shadow Crystal]\x01\nShoot a projectile that blinds and slows humans!\nRadius: %d\nCooldown: %ds\nUses: %d", range, base_script.cooldown, uses_left))
}

function ItemReady() {
	ClientPrint(base_script.item_holder, 3, "\x0768F9C4[Item] Shadow Crystal is ready!")
}
	...
	projectile_script.HitPlayer <- function(ent) {
		if (ent.GetTeam() == TEAM_HUMANS) {
			for (local ent; ent = Entities.FindByClassnameWithin(ent, "player", self.GetCenter(), range);) {
				...
				// Blind them slightly
				ScreenFade(ent, 32, 32, 32, 225, 1, 10, 1)

				// Slow them slightly
				SetPlayerSpeed(ent, 0.9, 10)
			}
