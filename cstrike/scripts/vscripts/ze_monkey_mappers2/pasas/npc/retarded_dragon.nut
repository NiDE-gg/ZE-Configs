...
idle_snd <- "monkeymappers/retarded_dragon_idle.mp3"
death_snd <- "monkeymappers/retarded_dragon_death.mp3"
...
function DisplayHealth() {
	if (physbox.IsValid())
		ClientPrint(activator, 4, format("Retarded Dragon HP: %d", physbox.GetHealth()))
}

function Init() {
	for (local ent = self.FirstMoveChild(); ent != null; ent = ent.NextMovePeer()) {
		if (ent.GetClassname() == "func_physbox_multiplayer") physbox = ent
		if (ent.GetClassname() == "prop_dynamic_override") model = ent
...
function Die(activator) {
	ClientPrint(null, 3, format("\x07FFAAAAA retarded dragon was slain by %s!", GetPlayerName(activator)))

	dead = true

	local sequence = model.LookupSequence("die")
