...
idle_snd <- "monkeymappers/steve_idle.mp3"
hurt_snd <- "monkeymappers/steve_hurt.mp3"
...
function NPCOnTakeDamage(activator) {
	...
	EmitSoundEx({ sound_name = hurt_snd, channel = 0, sound_level = 120, entity = self})
}
...
function DisplayHealth() {
	if (physbox.IsValid())
		ClientPrint(activator, 4, format("Steve HP: %d", physbox.GetHealth()))
}
...
function Die(activator) {
	ClientPrint(null, 3, format("\x07FFAAAASteve was slain by %s.", GetPlayerName(activator)))
