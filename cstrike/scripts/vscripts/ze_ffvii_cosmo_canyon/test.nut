this = EntityThisFixer

/* CS:S STRIPPER FIX
    Visual Health Bar Desync
    (SINCE THE BOSS ONLY DIES IF THE HEALTH BAR IS 0)
    Old Logic: "Only tell the game to change the health sprite when a segment breaks."
    New Logic: "Tell the game exactly which sprite to show based on the percentage of total health remaining, every 0.1 seconds."
*/

//CS:S FIX THE MODEL HAMMER MOVING MECHANIC WAS CHANGED!
//Since cs:s couldn'T handle the distance + the cubic moving phys i recreated that
//Since also couldn't the track_train clearparent what cause teleport it outside the map or if i backdoor the clraparent the physbox became buggy and start bouncing everywhere so a fake model was added and the normal was moved below
...
// ▚▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▞
{
    //CS:S STRIPPER FIX ADDED FLOAT VALUES
    HP <- 5.0;
    HP_INIT <- 5.0;
    HP_BARS <- 16;
...
// ▚▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▞
{
    //PUNCH
    {
        ...
        function Cast_Punch()
        {
            ...
            SetPlayBackRate(2);
            EntFireByHandle(self, "CallScriptFunction", "Cast_Punch_Next", 0.8, null, null);
        }
    ...
    //SHIELD
    {
        ...
        Shield_Random <- 0; //CS:S FIX MAYBE wrong (CS:GO value 1)
        ...
        function Cast_Shield()
        {
            ...
            EntFireByHandle(self, "CallScriptFunction", "Cast_Shield_Next", 1, null, null);
        }
        ...
        function Cast_Shield_Next()
        {
            ...
            {
                EntFireByHandle(self, "CallScriptFunction", "Cast_Spawn_Shield", time, null, null);
                time += timeadd;
            }
        }
    ...
    //ROCKET
    {
        ...
        function Cast_Rocket()
        {
            ...
            EntFireByHandle(self, "CallScriptFunction", "Cast_Rocket_Next", 1.2, null, null);
        }
        ...
        function Cast_Rocket_Spam()
        {
            ...
            Cast_Mine_Next();
            EntFireByHandle(self, "CallScriptFunction", "Cast_Rocket_Spam", 5, null, null);
        }
    }
    //LASER
    {
        ...
        function Cast_Laser_C()
        {
            ...
            EntFireByHandle(self, "CallScriptFunction", "Cast_Laser_C_Next", 1, null, null);
        }
            ...
            switch  (RandomInt(0,2))
                {
                    case 0:
                    {
                        EntFireByHandle(self, "CallScriptFunction", "Cast_Flang_Flame", 0, null, null);
                        break;
                    }
                    case 1:
                    {
                    ...
                    case 2:
                    {
                        EntFireByHandle(self, "CallScriptFunction", "Cast_Low_Flame", 0, null, null);
                        break;
                    }
    ...
    //MINE
    {
        ...
        function Cast_Mine()
        {
            ...
            EntFireByHandle(self, "CallScriptFunction", "Cast_Mine_Next", 0.8, null, null);
        }
    ...
    //HEAL
    {
        ...
        function Cast_Heal()
        {
            ...
            AddHP(CountAlive()*58); //CS:S HEALTH (CS:GO value 50)
        }
    }
    //CONFUSE
    {
        ...
        function Cast_Confuse()
        {
            ...
            EntFireByHandle(self, "CallScriptFunction", "Cast_Confuse_Next", 2.0, null, null);
            EntFire("AirBuster_Move_Physbox","EnableMotion","",3.4);
        }
     ...
     //Ice
    {
        ...
        function Cast_Ice()
        {
            ...
            EntFireByHandle(self, "CallScriptFunction", "Cast_Ice_Next", 2.0, null, null);
            EntFire("AirBuster_Move_Physbox","EnableMotion","",3.4);
        }
...
function PreStart()
{
    ...
    //CS:S FIX CAMERA
    //EntFire("Camera_old", "RunScriptCode", "SpawnCameras(Vector(-1250,5152,326),Vector(-60,180,0),6,Vector(-854,5152,486),Vector(15,180,0),3,4)", 11); //CS:S TEST 13
    EntFire("Camera_old", "RunScriptCode", "SpawnCameras(Vector(-1250,5152,326),Vector(-60,180,0),0,Vector(-1250,5152,326),Vector(-60,180,0),0,6,150)", 11);
    EntFire("Camera_old", "RunScriptCode", "SpawnCameras(Vector(-1250,5152,326),Vector(-45,180,0),0,Vector(-1250,5152,326),Vector(-45,180,0),0,1,150)", 11+6);
    EntFire("Camera_old", "RunScriptCode", "SpawnCameras(Vector(-1250,5152,326),Vector(-30,180,0),0,Vector(-1250,5152,326),Vector(-30,180,0),0,1,150)", 11+7);
    EntFire("Camera_old", "RunScriptCode", "SpawnCameras(Vector(-1250,5152,326),Vector(-15,180,0),0,Vector(-1250,5152,326),Vector(-15,180,0),0,1,150)", 11+8);
    EntFire("Camera_old", "RunScriptCode", "SpawnCameras(Vector(-1250,5152,326),Vector(0,180,0),0,Vector(-1250,5152,326),Vector(0,180,0),0,1,150)", 11+9);
    EntFire("Camera_old", "RunScriptCode", "SpawnCameras(Vector(-854,5152,486),Vector(15,180,0),0,Vector(-854,5152,486),Vector(15,180,0),0,3,150)", 11+10); //has to ber 3
    EntFire("Camera_old", "RunScriptCode", "SetOverLay(Overlay)", 11);
    EntFire("Camera_old", "CallScriptFunction", "SetOverLay", 24);
    //o1, a1, time1, o2, a2, time2, flytime
}
Default_Anim <- "walk";
function Start()
{
    EntFire("AirBuster_Move_Physbox", "AddOutPut", "overridescript damping,5,rotdamping,5", 0.02); //CS:S FIX if this set directly on prop_physic to the model the clearparent gonna tp him outside from the map
    EntFire("AirBuster_Nade", "Enable", "", 0);
    EntFireByHandle(self, "CallScriptFunction", "Cast_Rocket_Spam", 150, null, null);
    ...
    Tick();
    EntFireByHandle(self, "CallScriptFunction", "StunUnlock", RandomInt(30,50), null, null);
...
{
    Stanblock = false;
    ClientPrint(null, 3, Chat_pref + "The boss is weak, you can stagger him now!");
}
...
//        FakeParentArray.push(fakeparent(ent, "C_VFXMuzzleD_a", Vector(0, 0, 0), Vector(0, 200, 0)));
        ...
        SetHP(690); //CS:S HEALTH (CS:GO value 600)
    ...
    function Tick()
    {
        ...
        EntFireByHandle(self, "CallScriptFunction", "Tick", tickrate, null, null);
    }
    ...
    function UpDateHP()
    {
        ...
        if(HP_BARS <= 0)
        {
            ...
            EntFire("map_brush", "CallScriptFunction", "Trigger_Cave_After_Boss", 0);
        }
    }*/

    function UpDateHP()
    {
        // 1. Handle Bar Transitions (Moving from one bar to the next)
        if(HP < 1 && HP_BARS > 0)
        {
            HP += HP_INIT;
            HP_BARS--;
        ...
        // 2. Continuous Visual Update (Calculates exactly which texture index to show)
        local total_current = (HP_BARS * HP_INIT) + HP;
        local total_max = 16 * HP_INIT; // Assuming 16 is your visual max

        // Calculate index (0 to 16) based on TOTAL remaining health percentage
        local id = 16 - ((total_current / total_max) * 16).tointeger();

        // Clamp it so it doesn't break the texture limits
        if(id > 16) id = 16;
        if(id < 0) id = 0;
...
//        }
    ...
    function PrintAtt()ClientPrint(null, 3, AttachmenntArray[AttachmenntID]);
