...
function OnPostSpawn(){
	...
	maxhealth = HITBOX.GetHealth()+(112*ctcount) // 300 starting hp + 200/per player
    ...
    MODEL.GetScriptScope().ThinkHurt <- function(){
		local checkpos = self.GetOrigin()+Vector(0, 0, 24)+self.GetForwardVector()*offset;
...
function Attack(){
	...
	EntFireByHandle(MODEL, "RunScriptCode", "damage=40", 0, null, null)	
	...
	switch (RandomInt(1,4)) {
		case 1:
			EntFireByHandle(MODEL, "SetAnimation", "attack_01", 0, null, null);
			...
			for(local ii =0.40; ii <= 0.66; ii += 0.01){
			...
			for(local iii =1.30; iii <= 1.65; iii += 0.01){
				EntFireByHandle(self, "RunScriptCode", "MoveToEnemyForce(150, 150, 0)", iii, null, null);
			}
			EntFireByHandle(MODEL, "RunScriptCode", "active=false", 1.65, null, null)

			EntFireByHandle(self, "CallScriptFunction", "AttackingOff", 2.25, null, null);
			break;
		case 2:
			EntFireByHandle(MODEL, "SetAnimation", "attack_02", 0, null, null);

			for(local i =0.0; i <= 2.2; i += 0.01){
				EntFireByHandle(self,"CallScriptFunction","MonsterLookat",i,null,null);
			}

			EntFireByHandle(MODEL, "RunScriptCode", "active=true", 0.15, null, null)		
			...
			EntFireByHandle(MODEL, "RunScriptCode", "active=true", 0.9, null, null)	
			...
			EntFireByHandle(self, "CallScriptFunction", "AttackingOff", 2.4, null, null);
			break;
		case 3:
			EntFireByHandle(MODEL, "SetAnimation", "attack_03", 0, null, null);

			for(local i =0.0; i <= 0.15; i += 0.01){
				EntFireByHandle(self,"CallScriptFunction","MonsterLookat",i,null,null);
			}

			EntFireByHandle(MODEL, "RunScriptCode", "active=true", 0.15, null, null)	
			for(local ii =0.15; ii <= 0.35; ii += 0.01){
				EntFireByHandle(self, "RunScriptCode", "MoveToEnemyForce(200, 200, 0,true)", ii, null, null);
			}
			EntFireByHandle(MODEL, "RunScriptCode", "active=false", 0.75, null, null)

			EntFireByHandle(self, "CallScriptFunction", "AttackingOff", 1.35, null, null);
			break;
		default:
			EntFireByHandle(MODEL, "SetAnimation", "attack_strong", 0, null, null);

			EntFireByHandle(MODEL, "RunScriptCode", "damage=75", 0.1, null, null)	
			EntFireByHandle(MODEL, "RunScriptCode", "damage_range=48", 0.1, null, null)	
			EntFireByHandle(MODEL, "RunScriptCode", "offset=64", 0.1, null, null)	
			...
			for(local ii =0.15; ii <= 1.25; ii += 0.01){
				EntFireByHandle(self, "RunScriptCode", "MoveToEnemyForce(125, 125, 0,true)", ii, null, null);
			}
			EntFireByHandle(MODEL, "RunScriptCode", "active=false", 1.35, null, null)

			EntFireByHandle(self, "CallScriptFunction", "AttackingOff", 2.5, null, null);
			break;
	}
