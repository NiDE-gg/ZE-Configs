...
PrecacheModel("models/hob_cv/hobcv_dog.mdl")
PrecacheModel("models/hob_cv/hobcv_minotaur.mdl")
PrecacheModel("models/hob_cv/hobcv_gargoyle.mdl")
...
function OnPostSpawn(){
	...
	HITBOX.AcceptInput("ClearParent", "", null, null)
	...
	EntFireByHandle(MODEL, "SetAnimation", "idle", 0.2, null, null);
	PrecacheSound("npc/fast_zombie/claw_strike1.wav");
	EntFireByHandle(self,"RunScriptCode","Spawn()",0.1,null,null);
	EntFireByHandle(HITBOX,"Break","",60,null,null);
}

function Spawn(){
	local randommonsterint = RandomInt(1, 4)
	local hitboxorigin = HITBOX.GetOrigin()
	local hmin = Vector(-1,-1,-1)
	local hmax = Vector(1,1,1)
	
	local skeletonmaker;
	skeletonmaker = Entities.FindByName(null, "stage1_skeleton_maker")

	switch (randommonsterint) {
		case 1: // DOG NPC
			hmin = Vector(-49, -17, -21)
			hmax = Vector(49, 17, 21)
			MODEL.SetModelScale(1.0,0)
			MODEL.SetModel("models/hob_cv/hobcv_dog.mdl")
			npc_selected = 1
			break;
		case 2: // MINOTAUR NPC
			hmin = Vector(-37, -41, -47)
			hmax = Vector(37, 41, 47)
			MODEL.SetModelScale(0.55,0)
			MODEL.SetModel("models/hob_cv/hobcv_minotaur.mdl")
			HITBOX.SetOrigin(hitboxorigin+Vector(0,0,40))
			npc_selected = 2
			break;
		case 3: // GARGOYLE NPC
			...
			break;
		case 4:
			skeletonmaker.SpawnEntityAtEntityOrigin(self)
			EntFireByHandle(HITBOX,"Kill","",0.02,null,null);
			EntFireByHandle(self,"Kill","",0.04,null,null);
			break;
	}

	HITBOX.SetSize(hmin, hmax)
	EntFireByHandle(HITBOX, "SetParent", "!activator", 0.1, MODEL, null);
	EntFireByHandle(MODEL, "Enable", "", 0.1, null, null);
	EntFireByHandle(self, "AddOutput", "movetype 3", 0.2, null, null); // Re-enable movement
}
...
function Death(){
	...
	EntFireByHandle(MODEL, "SetAnimation", "die", 0.05, null, null);
...
function Attack(){
	...
	if (Time() - last_hit_time >= attack_rate) {
...
function LookForEnemy(){
    ...
    local players = [];
    for(local h;h=Entities.FindByClassname(h,"player");){
		if(h==null||!h.IsValid()||h.GetTeam()!=3||h.IsAlive()==false||!IsLOS(h))continue;players.push(h)}
    	if(players.len()>0){
...
function MoveToLastEnemyPositon(){
	...
	TraceLinePlayersIncluded(Vector, Vector, handle)
