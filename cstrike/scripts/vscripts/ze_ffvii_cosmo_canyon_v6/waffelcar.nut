...
self.PrecacheModel("models/heavy/wheelchair_glow.mdl"); //CS:S FIX
...
tickrate <- 0.01;
tickrate_sound <- 20.0;
forwarddist <- 82; //CSS: FIX (cs:go value 50)
class car
{
    text = null;
    textmessage = null;
    ...
    constructor(invalid_, model_, driverbutton_, glow_model_)
    {
        this.invalid = invalid_;
        this.model = model_;
        this.driverbutton = driverbutton_;
        this.glow_model = glow_model_;
    ...
    function SetTextCreate()
    {
        ...
        this.text.__KeyValueFromInt("font", 8); //CS:S FIX FONT
    ...
    function SetTextPos()
    {
        if(this.text == null || textmessage == null)
            return;
        //CS:S FIX SETPARENT BUG
        for (local j = 0; j < MAX_WEAPONS; j++)
        {
            local weapon = NetProps.GetPropEntityArray(this.invalid, "m_hMyWeapons", j)
            if (weapon.GetSlot() != 2) continue
            EntFireByHandle(this.text, "SetParent", "!activator", 0.01, weapon, null);
            break;
        }
        ...
        this.text.SetOrigin(this.invalid.GetOrigin() + (this.invalid.GetUpVector() * 48) + (this.invalid.GetLeftVector() * ((this.textmessage.len() * 3.0) * (this.textscale * 0.1))) + (this.invalid.GetForwardVector() * 7));
    ...
    {
        //SICNE THERE'S ONLY type 1 I'm bothering making more types
        //CS:S FIX SETPARENT BUG
        this.glow_model.__KeyValueFromString("rendercolor", newcolor);
        EntFireByHandle(this.glow_model, "FireUser3", "", 0.05, null, null);
    ...
    function SelfDestroy()
    {
        ...
        if(this.model != null && this.model.IsValid())
            this.model.Destroy();
        if(this.glow_model != null && this.glow_model.IsValid())
            this.glow_model.Destroy();
...
function AddCar()
{
    ...
    for (local j = 0; j < MAX_WEAPONS; j++)
    {
        local weapon = NetProps.GetPropEntityArray(activator, "m_hMyWeapons", j)
        if (weapon.GetSlot() != 2) continue
        EntFireByHandle(ent, "SetParent", "!activator", 0.01, weapon, null);
        break;
    }
    ent.SetOrigin(activator.GetOrigin() + activator.GetUpVector() * -32);
    ent.SetAngles(0, activator.GetAngles().y, 0);
    //Set up button
    local fix = caller.GetMoveParent();
    fix.SetOrigin(activator.GetOrigin() + activator.GetForwardVector() * 64 + activator.GetUpVector() * 7);
    ...
    //CS:S FIX ADD GLOW
    local nigger_model = SpawnEntityFromTable("prop_dynamic",
    {
        model = "models/heavy/wheelchair_glow.mdl",
        solid = 0,
        modelscale = 1.02, //SET GLOW SCALE
        disableshadows = 1,
        fademindist = 128,
        fademaxdist = 384, //value changed
        "OnUser2#1": "!selfTurnOff0-1"
        "OnUser3#1": "!selfTurnOn0-1"
    });
    for (local j = 0; j < MAX_WEAPONS; j++)
    {
        local weapon = NetProps.GetPropEntityArray(activator, "m_hMyWeapons", j)
        if (weapon.GetSlot() != 2) continue
        EntFireByHandle(nigger_model, "SetParent", "!activator", 0.01, weapon, null);
        break;
    }
    nigger_model.SetOrigin(activator.GetOrigin() + activator.GetUpVector() * -32);
    nigger_model.SetAngles(0, activator.GetAngles().y, 0);
    EntFireByHandle(nigger_model, "FireUser2", "", 0.03, null, null);

    g_Array_Car.push(GetPresent(car(activator, ent, caller, nigger_model)));
...
function GetPresent(Car_Class)
{
    ...
    else if(Preset == 1)
    {
        ...
        Car_Class.SetGlowColor("0 40 80", 0); //CS:S FIX from vector to var
    ...
    else if(Preset == 31)
    {
        ...
        Car_Class.SetGlowColor("42 0 42", 0); //CS:S FIX from vector to var
        Car_Class.SetTextColor(Vector(0,255,255));
    ...
    //CS:S FIX JUMP DISABLE with new netprop
        NetProps.SetPropBool(activator, "pl.deadflag", true);
    }
    else if(Car_Class.allowjump)
    {
        NetProps.SetPropBool(activator, "pl.deadflag", false);
    }
    //////////////
...
{
    //CS:S -heavy cars are only awabile after 40s from roundtsart
    local car_class = GetClassByButton(caller);
    local driver_class = GetClassByInvalid(activator);

    //SetDriver(car_class, activator); //DEBUG GONNA BUG
...
function Tick()
{
    foreach(car_class in g_Array_CheckCar)
	{
        ...
        {
            //CS:S FIX PLAYER POSITION
            //car_class.invalid == The guy who have the car
            //car_class.driver == the user who pressed E
            car_class.invalid.SetOrigin(car_class.driver.GetOrigin() + car_class.driver.GetForwardVector() * forwarddist); //STRIPPER RE-ENABLED
            //car_class.invalid.SetOrigin(car_class.driver.GetOrigin() + car_class.driver.GetUpVector() * forwarddist);
            //////////
            //car_class.model.SetAngles(0, car_class.driver.GetAngles().y, 0); //CS:GO DISABLED
...
function Tick_Sound()
{
    ...
    EntFireByHandle(self, "CallScriptFunction", "Tick_Sound", tickrate_sound, null, null);
}
