/* 1_2 CHANGES
- Rremoved radar blocking script
- Changed wheelchair player position (backto normal)
- Fixed humans items can be infected
*/

//I tried to make the vscript+map the same as it was in cs:go
//I'm still not a programmer so don't harass me -heavy

//DON'T REMOVE THE INVALID FIREUSER4 FROM HAMMER IS GETTING USED LATER

//HEAVY's stuffs
/*
--Added some nlolight block on 3d skybox to fix shadows
-Fixed models getting deleted from the map
-postprocess_controller i see no purpose to keeping it cause it's using the default settings (so i didn't tried to replicate it)
-FIXED MISSING DECALS, PARTICLES AND OVERLAY TEXTURE i also tried to replace the old default cs:s tesxtures with the new cs:go one (Only those what had a different look like the pipes or the oldman jacket)
-CHANGED PARENTED FUNC_MOVELINEAR TO DOOR
-FIXED vagon doors + getting stuck
-point_viewcontrol_multiplayer has changed to point_viewcontrol since the entity doesn't exist in cs:s
-All glow has changed with an other prop_dynamic
-mapspawnnut needed for the non edict entites what has vscript to work
-Changed some RunScriptCode to CallScriptFunction for faster script
-Replicated FadeAndKill from CS:GO
-Reshaped jukebox playerclip
-Fully rewroted the data system and deleted all the logic_eventlisteners and other entityes/functions/codes
-All the ScriptPrintMessageChatAll was changed to ClientPrint and i also changed the colors  (I just used Eyedropper on a csgo color layer to get the hex codes can see it in the saycommand.nut)
-Changed ScriptPrintMessageCenterAll to ClientPrint 4
-Remade all crouch spot for cs.s height
-kojima_particle_03 was moved out from the template due the scrip could't start the particle cause it wasn't spawned (the trigger is ok) issue came from cs:go
-The 640_pain_down sprite was not working on cs:s so i added AnimatedTexture and more frames to implement the cs:go pulse, fademin/mac doesn't work on it u can enable World Space Glow but that's breaks $ignorez
-There's a lot lot more other vscript changes what i didn't documented
-Fixled lightning modes/texture bugs
-removed all item_clipping cause it was cs:go feature to prevent e-pick
-nattack model changed texture to color to make the particle work
-changed bosses health
- last reno path_track gpt rotated a bit cuase the bos not exactly stops at the middle of it
- I did some hitbox changes
- Disabled stay out on almost all fade to prevent the previous fade not getting disabled

-I can't change the gibs or mute the sound so when a metal break and ut has the OnBreak() function all 2 sound gets played but it sounds fine
-In the camerafix nut i put "try" beside those stuffs! if no camera exist in the map don't throw an error and crash the server
*/

//MAPPER REQUESTED STUFFS
/*
-- ADD CS:GO C4
-- Disable thirdperson on scorpion bossfight (disabled function + removed nocull from vmf to see outside)
-- Don't add the hp bar warning msg on nattack boss
-- Remove the player disconnecting function
*/

//BOSS HEALTH CHANGES u can change health search up //CS:S HEALTH
//THE ITEMS USAGE HEALTH DMG NOT INCREASED!!!
/*
STAGE 1
	nattack/nattack.nut +115%
STAGE 2
	scorpion/main.nut +115%
	cactus/cactus.nut +115%
STAGE 4
	nattack/nattack.nut +115%
	last_nattack/last_nattack.nut +115%
STAGE 5
	airbuster/test.nut +115%
	reno/reno.nut 1 phase +115 / 2 phase +115%
*/

//FADE CHANGES
/*
//All fade disabled stay out to prevent the previous fade not getting disabled making an error fade + spamming it //CS:S FIX v2
fade_white: 180 -> 25
fade_red: 180 -> 75
Poison_fade: 100 -> 2
travel_fade: The duration from 4 -> set to 1

//YUFFIE/MIKE FADE IS GETTING CREATED FROM HERE THE CHANGES ARE WRITEN AN THERE TOO
*/
IncludeScript("gfl/cosmov6/randomstuff/saycommands.nut")

PrecacheSound("heavy_csgo/container_death_01.wav")
PrecacheSound("heavy_csgo/container_death_02.wav")
PrecacheSound("heavy_csgo/container_death_03.wav")
...
//CS:S FIX entity cleaner since cs:s shit at entity cleaning (without this the server crash after new round. Cause the items create too many edicts if admin spawns too many new ones)
//Maybe maybe.. remove some entitys not all to make round end more good looking
...
//CS:S FIX since env_sprite_clientside doesn't exits the map crash after mapload cause too many edcits this spawns it after the map loaded then the edict limit goes back to 1300
//THIS GETTING CALLED BY LOGIC_AUTO
sprites_spawned <- false
function CSS_SPAWN_SPRITES()
{
    if (sprites_spawned) return;
    sprites_spawned = true;

    local ent = null;

    while ( (ent = Entities.FindByName(ent, "spawn_white_sprite")) != null )
    {
        local origin = ent.GetOrigin();

        local sprite = SpawnEntityFromTable("env_sprite", {
			targetname = "css_sprites"
            GlowProxySize = 8
            renderamt = 155
            rendermode = 9
            scale = 1
	...
	while ( (ent = Entities.FindByName(ent, "css_sprites_shitcolor")) != null )
    {
        local origin = ent.GetOrigin();

        local sprite = SpawnEntityFromTable("env_sprite", {
			targetname = "css_sprites_shit"
            GlowProxySize = 8
            renderamt = 100
            rendermode = 9
            scale = 1
            model = "sprites/glow_test02.vmt"
            rendercolor = "200 200 120 1500"
            spawnflags = 1
            origin = origin
        });
    }
...
//CS:S replicate MoveToPathNode
//Currently this only works on preserved entities
//If u do like OnPass test_relay:RunScriptCode:MoveToPathNode('test_train','test_path_2'):0:1 U gonna get this //InputRunScript line = (1) column = (27) : error constant too long//
//So highly recommended to don't use that and use EntFire("---", "RunScriptCode","MoveToPathNode(`---`,`---`);", ---, null);
function MoveToPathNode(train_name, path_name, train_direction = "StartForward")
{
    local train = Entities.FindByName(null, train_name);
    local target_path = Entities.FindByName(null, path_name);

	EntFireByHandle(train, train_direction, "", 0.0, null, null);

	//Since i can't het the current node from the func_track train i do this
	EntityOutputs.AddOutput(target_path,"OnPass","!activator","Stop","0",0,1); //Add output to stop the train instead of putting it to the think function -set it on 1 so i don't need remove output
	EntityOutputs.AddOutput(target_path,"OnPass","!activator","AddOutput","speed 200",0,1); //CS:S FIX BACKWARC CAUSE POOR CODING

	function CheckTrain()	//Ticking function
    {
        local prev_node = NetProps.GetPropEntity(train, "m_ppath"); //Get previous node
        local next_node = NetProps.GetPropEntity(prev_node, "m_pnext"); // Get previous node next node which is the current node and the train gonna stop here

        // Check if it's entering to next node
        if (prev_node == target_path)
        {
			//TODO FIX
			//EntityOutputs.RemoveOutput(target_path,"OnPass","!activator","Stop","",0,-1);
        }
        else
        {
            EntFireByHandle(self, "RunScriptCode", "CheckTrain()", 0.01, null, null);
        }
    }
    CheckTrain();
}

//CS:S replicate FadeAndKill
function FadeAndKill(entity)
{
    local target = null;

	//Check if caller not given by specific name
    if (entity == caller)
    {
        target = caller;
    }
    ...
    {
        target = Entities.FindByName(null, entity);
    }
    ...
    function Fade_tick()
    {
        if(fadevalue > 0)
        {
            ...
            EntFireByHandle(target, "Alpha", "" + fadevalue, 0.0, null, null);
            EntFireByHandle(self, "CallScriptFunction", "Fade_tick", 0.01, null, null);
        }
        else
        {
            fadevalue = 0;
            ticking = false;
			EntFireByHandle(target, "Kill", "", 0.0, null, null);
        }
    }
    ...
    if (!ticking)
    {
        EntFireByHandle(target, "AddOutput", "rendermode 1", 0.0, null, null);
        ticking = true;
        Fade_tick();
    }
}
...
/*
CS:S FIX replace DestroyImmediately for info_particle_system
These particles need Movement Lock to Controlpoint so i added for the following paticles

-custom_particle_223 (Ice_Air_Effect)
-custom_particle_225 (Confuse_Effect)
*/
function DestroyImmediately(entity)
{
	local target = null;
	...
	target.SetOrigin(Vector(0 0 -13000)) //tp the particle to a secret position where nobody can see it
	//EntFireByHandle(target, "Kill", "", 0.0, null, null); these particle_systems using kill by them self so this is disabled

}

//TODO FIX METERIAL GIBS WHO USING THIS (CS:GO material type 9)
//CS:S FIX REPLICATE CUSTOM MATERIAL SOUNDS WHAT'RE NOT EXIST IN CS:S
::BreakSounds <- [
    "heavy_csgo/container_death_01.wav",
    "heavy_csgo/container_death_02.wav",
    "heavy_csgo/container_death_03.wav"
];
function OnBreak()
{
    local soundIndex = RandomInt(0, BreakSounds.len() - 1);
    EmitSoundOn(BreakSounds[soundIndex], caller);
}
...
{
	sprites_spawned = false
	EntFireByHandle(self, "CallScriptFunction", "CSS_SPAWN_SPRITES", 2, null, null);
	...
	g_hFadeItem.__KeyValueFromInt("renderamt", 20); //CS:GO VALUE 255
	g_hFadeItem.__KeyValueFromInt("spawnflags", 5);
	g_hFadeItem.__KeyValueFromString("targetname", "7"); //CS:S FIX MODULATE FADE DOESN'T WORK IN CS:S
	...
	else
	{
		...
		{
			//CS:S FIX classic settings pluss add some new
			EntFire("css_fix_spawn_classic*", "Kill", "", 0, null); //removing spawn text/sprites from perks
			EntFire("css_fix_classic", "Enable", "", 0, null); //Disable shop second fool
			EntFire("shop_travel_trigger", "Disable", "", 0, null); //Disable shop second fool
			///////////
		...
		EntFireByHandle(self, "CallScriptFunction", "NewStageGiveMoney", 0.5, null, null);
		...
		else if(Stage == 3)//zm
		{
			...
			EntFire("map_brush", "CallScriptFunction", "Trigger_ZM_Tp", 149.00);
...
function StartVote()
{
	...
	Vote_Players.clear();
	NIGGERS_ALIVE = 0; //CS:S set it back to 0 when start new vote
	//CS:S FIX v2 since CheckValidInArr was revorked the PLAYERS couldn't been cleared so just count players at roundstart
	for (local player = null; player = Entities.FindByClassname(player, "player");)
	{
		if(player!=null && player.IsValid() && player.GetHealth()>1)
		{
			NIGGERS_ALIVE++
		}
	}
	...
	{
		EntFireByHandle(self, "CallScriptFunction", "Vote_Tick", 5.00 + i, null, null);
	}
	EntFireByHandle(self, "CallScriptFunction", "Vote_End", 6.00 + i, null, null);
}
...
function Vote_Tick()
{
	...
	local TottalPlayers = (NIGGERS_ALIVE * Vote_NeedPlayers).tointeger();
	/////
...
function Vote_End()
{
	...
	if (VotePlayers == 0)
	{
		//CS:S fix added this to delet model if nobody vote
	...
	if (VotePlayers >= TottalPlayers)
	{
		...
		{
			EntFireByHandle(g_round, "FireWinCondition", "9", 0, null, null);
		}
	}
	EntFire("Classic_prop", "FireUser1", "", 0, null);
}
...
function Warmup_TICK()
{
	...
	else
	{
		...
		EntFireByHandle(self, "CallScriptFunction", "Warmup_TICK", 1.00, null, null);
	}
...
function MapReset()
{
	...
	DisableHudHint = true;
	//CS:S FIX MAYBE CAN CAUSE PROBLEMS
	...
	*/
	EntFire("Camera_old", "CallScriptFunction", "SetOverLay", 0);

	Shop_TP_Count = Shop_TP_CountB;

	EntFireByHandle(self, "CallScriptFunction", "MapFullRestart", 1.5, null, null);
}
...
"[U:1:248696175]",  //kotya
"[U:1:241854455]",  //kondik
"[U:1:116002616]",   //Mike
"[U:1:155364080]",	//Friend
"[U:1:40412677]",	//HaRyDe
];

VIP_STEAM_ID <-[
"[U:1:1023819726]", //heavy
"[U:1:35551385]",    //memes translate
"[U:1:350665621]",    //Champagne translate
"[U:1:229842349]",    //Koen  translate

"[U:1:322548191]",    //Niceshoot script help
"[U:1:45042565]",   //Luffaren script help
"[U:1:410330410]",    //waffel just waffel

"[U:1:318832497]",    //toppi made skin for VIP

"[U:1:72910852]",    //Lexer
"[U:1:64568989]",    //Jayson
"[U:1:353059392]",    //switchwwe
"[U:1:196152865]",   //xmin
"[U:1:62949877]",   //Headsh
"[U:1:112811694]",   //Vishnya

"[U:1:106502527]"    //SHUFEN
"[U:1:65932212]",    //extrim
"[U:1:187139329]",    //Kun
"[U:1:141413953]",    //Lunar
"[U:1:66073416]",    //CrazyKid
"[U:1:193607768]",    //Hestia
"[U:1:191103061]",    //ZeddY
"[U:1:135119155]",    //FireWork
"[U:1:104850620]",    //Nemo
"[U:1:137727934]",    //Nano
"[U:1:435397098]",    //Tianli

"[U:1:374036212]",    //creepy

"[U:1:1090052436]",    //sushibanana
"[U:1:135614266]",    //spxctator
"[U:1:369745157]",    //xiaodi
"[U:1:278001334]",    //liala
"[U:1:463177489]",    //takoyaki
"[U:1:43677704]",    //detroid
"[U:1:840147767]",    //shizuka
"[U:1:153037375]",    //tupu
"[U:1:366450511]",    //ponya
"[U:1:427776758]",    //burning my life
"[U:1:203438253]",    //bulavator
"[U:1:89446230]",    //umad
"[U:1:258368085]",    //dead angel
"[U:1:260197804]",    //Ruby

"[U:1:268897980]",    //Mist
"[U:1:329366153]",    //Tsukasa

"[U:1:243480644]",    //Ambitious
"[U:1:165320006]",    //tilgep
"[U:1:1204384080]",    //ump9

"[U:1:108893258]",    //FPT

"[U:1:391949861]",    //Igromen

"[U:1:108219257]",    //Cron

"[U:1:251966636]",    //widez

"[U:1:1128919548]",    //kiskis

"[U:1:376628145]",    //Crimson
"[U:1:103974714]",    //Tachibana

"[U:1:889363189]",    //M1mic

"[U:1:402394686]",    //WhiteShadow
"[U:1:84984621]",    //Wind of Liberty
"[U:1:1240521100]",    //xz china nickname
"[U:1:66138566]",    //Datless

"[U:1:282351436]",   //priv ii
"[U:1:66482081]",    //Frygtlos

"[U:1:847980984]",   //dolo
];

/* -heavy
The number after the steam id needed cause that's how people get their custom stuffs
*/
INVALID_STEAM_ID <- [
"[U:1:1023819726] 2", //heavy
"[U:1:112811694] 27",    //Vishnya
"[U:1:40412677] 22",    //HaRyDe
"[U:1:421145217] 0"     //Shy Way
"[U:1:861684714] 0",    //naiz
"[U:1:248696175] 1",      //kotya
"[U:1:62949877] 0",        //headshoter
"[U:1:196152865] 0",        //xmin
"[U:1:35551385] 3",        //memes
"[U:1:1122654292] 0",    //INSIDE
"[U:1:410330410] 2",    //waffle
"[U:1:116002616] 4",        //Mike
"[U:1:65932212] 0",        //extrim
"[U:1:107170794] 0",        //Imma
"[U:1:350665621] 5",    //Champagne
"[U:1:296295212] 6",    //mrs. Champagne
"[U:1:374036212] 7",    //Creepy
"[U:1:268897980] 8",    //Mist

"[U:1:1090052436] 9",    //sushibanana
"[U:1:135614266] 10",    //spxctator
"[U:1:369745157] 11",    //xiaodi
"[U:1:278001334] 12",    //liala
"[U:1:463177489] 13",    //takoyaki
"[U:1:43677704] 14",    //detroid
"[U:1:840147767] 15",    //shizuka
"[U:1:153037375] 16",    //tupu
"[U:1:366450511] 17",    //ponya
"[U:1:427776758] 18",    //burning my life
"[U:1:203438253] 19",    //bulavator
"[U:1:89446230] 20",    //umad
"[U:1:258368085] 21",    //dead angel

"[U:1:243480644] 29",    //Ambitious
"[U:1:165320006] 28",    //tilgep
"[U:1:1204384080] 23",    //ump9

"[U:1:104850620] 24",    //Nemo
"[U:1:108893258] 25",    //FPT
"[U:1:391949861] 26",    //Igromen

"[U:1:1103335171] 30",    //Bonesaw
"[U:1:237290198] 31",    //MercaXlv

"[U:1:108219257] 32",    //Cron

"[U:1:1128919548] 33",    //kiskis

"[U:1:376628145] 34",    //Crimson
"[U:1:103974714] 35",    //Tachibana
"[U:1:435579692] 36",    //Kurumi

"[U:1:889363189] 37",    //M1mic

"[U:1:402394686] 38",    //WhiteShadow
"[U:1:84984621] 39",    //Wind of Liberty
"[U:1:1240521100] 40",    //xz china nickname
"[U:1:66138566] 41",    //Datless
];
...
perkhp_zm_cost <- 45;
::perkhp_zm_hpperlvl <- 3750; //CS:S FIX IF EVERY ZM START WITH 7500 and we want 30000 the value changed (CS:GO 4500)
...
::perkhp_hm_hpperlvl <- 20; //CS:S FIX LVL to make it 300hp u have to set it 20 (CS:GO 25)
...
class Player
{
	...
	//CS:S FIX ADDED HANDLE INSTEAD OF proxy from csgo
	constructor(_userid,_name,_steamid,_handle)
	{
		this.userid = _userid;
		this.name = _name;
...
function BuyStock()
{
	...
	//CS:S FIX stock money
	//local money = stocklen * (lvlprice - lvlprice * 0.2);
	local money = (stock.len()-1) * lvlprice * 0.8
	...
	if(name.find("ammo") != null)
	{
		if(item_ammo_count > 0)
		{
			...
			if(pl.money >= needmoney)
			{
				...
				if(item_ammo_count == 0)
					//CS:S GLOW FIX
					EntFire("info_glow_ammo_glow", "Kill", "", 0, null);
				return;
			}
		}
		else
	...
	else if(name.find("potion") != null)
	{
		if(item_potion_count > 0)
		{
			...
			if(pl.money >= needmoney)
			{
				...
				if(item_potion_count == 0)
					//CS:S GLOW FIX
					EntFire("info_glow_potion_glow", "Kill", "", 0, null);
				return;
			}
	...
	else if(name.find("phoenix") != null)
	{
		if(item_phoenix_count > 0)
		{
			...
			if(pl.money >= needmoney)
			{
				...
				if(item_phoenix_count == 0)
					//CS:S GLOW FIX
					EntFire("info_glow_phoenix_glow", "Kill", "", 0, null);
				return;
			}
...
function BuyPerk()
{
	//CS:S FIX ALL THE BUYPERKS. ON CS:GO ALL WAS BROKEN
	//Most buyberk doesn't have more expensive price the only price change is with huckster but that's making the price less expensive
	local name = caller.GetName();
	local pl = GetPlayerClassByHandle(activator);
	local needmoney = null;
	//CS:S FIX PERKS LIMITS WHAT WASN'T FIXED IN CS:GO
	...
	else if(name.find("hp_zm") != null)
	{
		...
		local maxlvl = perkhp_zm_maxlvl; //CS:S FIX ZM HEALTH (CS:GO perkhp_hm_maxlvl)
		local defaultcost = perkhp_zm_cost; //CS:S FIX COST (CS:Go perkhp_hm_cost)
		local cost = ((lvl > maxlvl) ? (defaultcost * 2) : defaultcost);
		///
		if(lvl == maxlvl)
		{
	...
	else if(name.find("huckster") != null)
	{
		...
		needmoney = perkhuckster_cost; //cs:s this doesn't neeed GetNewPrice since this makes other perks less expensive
		if(pl.money >= needmoney)
		{
...
function InfoStock()
{
	...
	//local money = stock.len() * (lvlprice - lvlprice * 0.2);
				//item count /maxed item price / discound -20%
	local money = (stock.len()-1) * lvlprice * 0.8
	//printl(lvlprice) ///225 allways DEBUG
	//printl(stock.len()); //if 1 item the value is 2 //// if 2 item the value is 3 DEBUG
	//printl(money) //result DEBUG
...
function PickUpItem()
{
	...
	if(name.find("bio") != null)
	{
		...
		item_name = "\x07B0C2D8 Bio\x07FFFFFF ";
	...
	else if(name.find("ice") != null)
	{
		...
		item_name = "\x075E98D8 Ice\x07FFFFFF ";
	}
	else if(name.find("poison") != null)
	{
		lvl = pl.poison_lvl;
		...
		item_name = "\x07A3FE47 Poison\x07FFFFFF ";
	}
	else if(name.find("wind") != null)
	{
		lvl = pl.wind_lvl;
		...
		item_name = "\x07BFFD8E Wind\x07FFFFFF ";
	}
	else if(name.find("summon") != null)
	{
		lvl = pl.summon_lvl;
		...
		item_name = "\x07ECE37A Summon\x07FFFFFF ";
	}
	else if(name.find("fire") != null)
	{
		lvl = pl.fire_lvl;
		...
		item_name = "\x07FE0000 Fire\x07FFFFFF ";
	}
	else if(name.find("electro") != null)
	{
		lvl = pl.electro_lvl;
		...
		item_name = "\x074B69FF Electro \x07FFFFFF ";
	}
	else if(name.find("earth") != null)
	{
		lvl = pl.earth_lvl;
		...
		item_name = "\x07E1AE39 Earth \x07FFFFFF ";
	}
	else if(name.find("gravity") != null)
	{
		lvl = pl.gravity_lvl;
		...
		item_name = "\x07D22EE5 Gravity\x07FFFFFF ";
	}
	else if(name.find("ultimate") != null)
	{
		lvl = pl.ultimate_lvl;
		...
		item_name = "\x0740FF3D Ultima\x07FFFFFF ";
	}
	else if(name.find("heal") != null)
	{
		lvl = pl.heal_lvl;
	...
	if (havesupport)
	{
		//CS:S  FIX this not needed since new glow system
		for(local i = 1; i <= lvl;i++)
		{
	...
	if (ENT_WATCH_ENABLE)
	{
		...
		ClientPrint(null, 3, "\x07FFFFFF***\x0740FF3D "+pl.name+"\x07FFFFFF {\x07FD3F3F"+pl.steamid+"\x07FFFFFF} picked up"+ item_name+""+color+""+lvl+"\x07FFFFFF level ***")
	}
		...
		if(ENT_WATCH_ENABLE)
		{
			...
			if(pl != null)
			{
				...
				if(item_owner.name_right == "phoenix")
					item_name = " Phoenix down ";
				ClientPrint(null, 3, "\x07FFFFFF***\x0740FF3D "+pl.name+"\x07FFFFFF {\x07FD3F3F"+pl.steamid+"\x07FFFFFF} dropped"+item_name+" "+((lvl==3) ? RED : (lvl==2) ? YELLOW : GREEN)+""+((lvl != null) ? lvl : 0)+"\x07FFFFFF level ***")
			}
		}
	}
}
...
function ShowItems()
{
	...
	EntFireByHandle(self, "CallScriptFunction", "ShowItems", 0.5, null, null);
}
...
function UseUltimate()
{
	...
	if (ENT_WATCH_ENABLE)
	{
		ClientPrint(null, 3, "\x07FFFFFF***\x0740FF3D "+pl.name+"\x07FFFFFF {\x07FD3F3F"+pl.steamid+"\x07FFFFFF} used\x0740FF3D Ultima\x07FFFFFF "+((lvl==3) ? RED : (lvl==2) ? YELLOW : GREEN)+""+((lvl != null) ? lvl : 0)+"\x07FFFFFF level ***");
	}
...
function UseWind()
{
	...
	if (ENT_WATCH_ENABLE)
	{
		ClientPrint(null, 3, "\x07FFFFFF***\x0740FF3D "+pl.name+"\x07FFFFFF {\x07FD3F3F"+pl.steamid+"\x07FFFFFF} used\x07BFFD8E Wind\x07FFFFFF "+((lvl==3) ? RED : (lvl==2) ? YELLOW : GREEN)+""+((lvl != null) ? lvl : 0)+"\x07FFFFFF level ***");
	}
	...
	EntFire("item_trigger_wind"+postfix, "CallScriptFunction", "Enable", 0.01, null);

	EntFire("item_trigger_wind"+postfix, "CallScriptFunction", "Disable", worktime, null);
	// if(pl.item_buff_doble)
...
function UseIce()
{
	...
	if (ENT_WATCH_ENABLE)
	{
		ClientPrint(null, 3, "\x07FFFFFF***\x0740FF3D "+pl.name+"\x07FFFFFF {\x07FD3F3F"+pl.steamid+"\x07FFFFFF} used\x075E98D8 Ice\x07FFFFFF "+((lvl==3) ? RED : (lvl==2) ? YELLOW : GREEN)+""+((lvl != null) ? lvl : 0)+"\x07FFFFFF level ***");
	}
	if(!item_owner.canUse)
	{
		EntFire("item_effect_"+item_preset.name+""+postfix, "Stop", "", 0, null);
...
function UseElectro()
{
	//CS:S FIX changed text color transparency (CS:GO color1 7 50 248) 170 185 255
	local item_owner = GetItemByButton(caller);
	local postfix = item_owner.name;
	...
	if (ENT_WATCH_ENABLE)
	{
		ClientPrint(null, 3, "\x07FFFFFF***\x0740FF3D "+pl.name+"\x07FFFFFF {\x07FD3F3F"+pl.steamid+"\x07FFFFFF} used\x074B69FF Electro\x07FFFFFF "+((lvl==3) ? RED : (lvl==2) ? YELLOW : GREEN)+""+((lvl != null) ? lvl : 0)+"\x07FFFFFF level ***");
	}
...
function UseSummon()
{
	...
	if (ENT_WATCH_ENABLE)
	{
		ClientPrint(null, 3, "\x07FFFFFF***\x0740FF3D "+pl.name+"\x07FFFFFF {\x07FD3F3F"+pl.steamid+"\x07FFFFFF} used\x07ECE37A Summon\x07FFFFFF "+((lvl==3) ? RED : (lvl==2) ? YELLOW : GREEN)+""+((lvl != null) ? lvl : 0)+"\x07FFFFFF level ***");
	}
	if(!item_owner.canUse)
	{
		EntFire("item_effect_"+item_preset.name+""+postfix, "Stop", "", 0, null);
	...
	EntFire("item_temp_summon_trigger"+name, "CallScriptFunction", "SelfDestroy", Worktime, null);
	Boss_Damage_Item(item_owner.name_right, (!pl.item_buff_radius) ? lvl : (lvl <= 1) ? 1 : lvl - 1);
}
...
function UseBio()
{
	...
	if (ENT_WATCH_ENABLE)
	{
		ClientPrint(null, 3, "\x07FFFFFF***\x0740FF3D "+pl.name+"\x07FFFFFF {\x07FD3F3F"+pl.steamid+"\x07FFFFFF} used\x07B0C2D8 Bio\x07FFFFFF "+((lvl==3) ? RED : (lvl==2) ? YELLOW : GREEN)+""+((lvl != null) ? lvl : 0)+"\x07FFFFFF level ***");
	}
	if(!item_owner.canUse)
	{
		EntFire("item_effect_"+item_preset.name+""+postfix, "Stop", "", 0, null);
...
function UsePoison()
{
	...
	if (ENT_WATCH_ENABLE)
	{
		ClientPrint(null, 3, "\x07FFFFFF***\x0740FF3D "+pl.name+"\x07FFFFFF {\x07FD3F3F"+pl.steamid+"\x07FFFFFF} used\x07A3FE47 Poison\x07FFFFFF "+((lvl==3) ? RED : (lvl==2) ? YELLOW : GREEN)+""+((lvl != null) ? lvl : 0)+"\x07FFFFFF level ***");
	}
	if(!item_owner.canUse)
	{
	...
	EntFire("item_temp_poison_trigger"+name, "CallScriptFunction", "SelfDestroy", 20.0, null);
...
function UseEarth()
{
	//CS:S FIX PARENTED THE earth_effect in hammer cause it was unparented what prevented disappering the model
	local item_owner = GetItemByButton(caller);
	local postfix = item_owner.name;
	...
	if (ENT_WATCH_ENABLE)
	{
		ClientPrint(null, 3, "\x07FFFFFF***\x0740FF3D "+pl.name+"\x07FFFFFF {\x07FD3F3F"+pl.steamid+"\x07FFFFFF} used\x07E1AE39 Earth\x07FFFFFF "+((lvl==3) ? RED : (lvl==2) ? YELLOW : GREEN)+""+((lvl != null) ? lvl : 0)+"\x07FFFFFF level ***");
	}
	if(!item_owner.canUse)
	{
		EntFire("item_effect_"+item_preset.name+""+postfix, "Stop", "", 0, null);
...
function UseGravity()
{
	...
	if (ENT_WATCH_ENABLE)
	{
		ClientPrint(null, 3, "\x07FFFFFF***\x0740FF3D "+pl.name+"\x07FFFFFF {\x07FD3F3F"+pl.steamid+"\x07FFFFFF} used\x07D22EE5 Gravity\x07FFFFFF "+((lvl==3) ? RED : (lvl==2) ? YELLOW : GREEN)+""+((lvl != null) ? lvl : 0)+"\x07FFFFFF level ***");
	}
	if(!item_owner.canUse)
	{
		EntFire("item_effect_"+item_preset.name+""+postfix, "Stop", "", 0, null);
	...
	EntFire("item_temp_gravity_trigger"+name, "CallScriptFunction", "Disable", worktime, null);
...
function UseFire()
{
	...
	if (ENT_WATCH_ENABLE)
	{
		ClientPrint(null, 3, "\x07FFFFFF***\x0740FF3D "+pl.name+"\x07FFFFFF {\x07FD3F3F"+pl.steamid+"\x07FFFFFF} used\x07FE0000 Fire\x07FFFFFF "+((lvl==3) ? RED : (lvl==2) ? YELLOW : GREEN)+""+((lvl != null) ? lvl : 0)+"\x07FFFFFF level ***");
	}
	if(!item_owner.canUse)
	{
		EntFire("item_effect_"+item_preset.name+""+postfix, "Stop", "", 0, null);
	...
	EntFire("item_trigger_fire"+postfix, "CallScriptFunction", "Enable", 0.01, null);

	EntFire("item_trigger_fire"+postfix, "CallScriptFunction", "Disable", worktime, null);
...
function UseHeal()
{
	...
	if (ENT_WATCH_ENABLE)
	{
		ClientPrint(null, 3, "\x07FFFFFF***\x0740FF3D "+pl.name+"\x07FFFFFF {\x07FD3F3F"+pl.steamid+"\x07FFFFFF} used Heal "+((lvl==3) ? RED : (lvl==2) ? YELLOW : GREEN)+""+((lvl != null) ? lvl : 0)+"\x07FFFFFF level ***");
	}
	if(!item_owner.canUse)
	{
		EntFire("item_effect_"+item_preset.name+""+postfix, "Stop", "", 0, null);
	...
	EntFire("item_trigger_heal"+postfix, "CallScriptFunction", "Enable", 0.01, null);

	EntFire("item_trigger_heal"+postfix, "CallScriptFunction", "Disable", worktime, null);
...
function UsePotion()
{
	...
	if (ENT_WATCH_ENABLE)
	{
		ClientPrint(null, 3, "\x07FFFFFF***\x0740FF3D "+pl.name+"\x07FFFFFF {\x07FD3F3F"+pl.steamid+"\x07FFFFFF} used Potion "+((lvl==3) ? RED : (lvl==2) ? YELLOW : GREEN)+""+((lvl != null) ? lvl : 0)+"\x07FFFFFF level ***");
	}
	EntFire("item_sound_use", "PlaySound", "", 0, null);
	...
	EntFireByHandle(self, "RunScriptCode", "FadeAndKill(\"item_effect_" + item_preset.name + postfix + "\")", 0, null, null);

	EntFireByHandle(self, "RunScriptCode", "FadeAndKill(\"item_effect_potion" + postfix + "\")", 0, null, null);
	...
	EntFire("item_trigger_potion"+postfix, "CallScriptFunction", "Enable", 0.01, null);

	EntFire("item_trigger_potion"+postfix, "CallScriptFunction", "Disable", worktime, null);
...
function UseAmmo()
{
	...
	if (ENT_WATCH_ENABLE)
	{
		ClientPrint(null, 3, "\x07FFFFFF***\x0740FF3D "+pl.name+"\x07FFFFFF {\x07FD3F3F"+pl.steamid+"\x07FFFFFF} used Ammo "+((lvl==3) ? RED : (lvl==2) ? YELLOW : GREEN)+""+((lvl != null) ? lvl : 0)+"\x07FFFFFF level ***");
	}
	EntFire("item_sound_use", "PlaySound", "", 0, null);
	...
	EntFire("item_trigger_"+item_preset.name+""+postfix, "CallScriptFunction", "Enable", 0.01, null);

	EntFire("item_trigger_"+item_preset.name+""+postfix, "CallScriptFunction", "Disable", worktime, null);
...
function UsePhoenix()
{
	...
	if (ENT_WATCH_ENABLE)
	{
		ClientPrint(null, 3, "\x07FFFFFF***\x0740FF3D "+pl.name+"\x07FFFFFF {\x07FD3F3F"+pl.steamid+"\x07FFFFFF} used Phoenix down "+((lvl==3) ? RED : (lvl==2) ? YELLOW : GREEN)+""+((lvl != null) ? lvl : 0)+"\x07FFFFFF level ***");
	}
	EntFire("item_sound_use", "PlaySound", "", 0, null);
	...
	if (item_owner.RemoveOwner())
	{
		if (ENT_WATCH_ENABLE)
		{
			...
			if(item_owner.name_right == "Yuffie")
				item_name = " Yuffie ";
			ClientPrint(null, 3, "\x07FFFFFF***\x0740FF3D "+pl.name+"\x07FFFFFF {\x07FD3F3F"+pl.steamid+"\x07FFFFFF} died and dropped"+item_name+" "+((lvl==3) ? RED : (lvl==2) ? YELLOW : GREEN)+""+((lvl != null) ? lvl : 0)+"\x07FFFFFF level ***")
		}
	}
}
		...
		switch (Active_Boss)
		{
			...
			else if(item_name == "ultimate")
			{
				if(lvl == 1)
					...
					damage = -4;
				EntFire(temp, "CallScriptFunction", "ItemEffect_Ultima", 0.00);
				EntFire(temp, "RunScriptCode", "PlaySound(Sound_Shit,1,1,0)", 0.00);
			}
...
::GetPetIndexByHandle <- function(handle)
{
...
({
	//CS:S FIX added player spawn instead of player_connect + info_game_event_proxy
	OnGameEvent_player_spawn = function(params)
	{
		...
		player.ValidateScriptScope();
		//SendToConsoleServerPS("sv_disable_immunity_alpha 1"); //not in css
		local steamid = ::GetPlayerSteamID(player);
		local name = NetProps.GetPropString(player, "m_szNetname");
		//Rewrited CheckValidInArr function (Honestly this just a check to prevent data duplication)
		//Delet the empty steam id when the player joins to the server
		if (steamid == null || steamid == "")
			return;

		//Check if player already exist then remove it if yes
		...
		if (existing_index != -1) {
			//If player already exist reuse the old one and update some data
			local existing = PLAYERS[existing_index];
			existing.userid = userid;
			existing.name = name;
			existing.handle = player;
		}
		else {
			//Rewrite some old parameters with old data if player disconnected but joined back
			local p_new = Player(userid, name, steamid, player);
		...
		//CS:S FIX reworked function ElseCheck()
		local pl = GetPlayerClassByHandle(player)
		if (pl != null)
		{
			if (MAPPER_STEAM_ID.find(steamid) != null)
			{
				pl.mapper = true;
				pl.vip = true;
				ShowPlayerText(pl,"Thanks for map")
			}
			else if (VIP_STEAM_ID.find(steamid) != null)
			{
				pl.vip = true;
				ShowPlayerText(pl,"Thanks for supporting")
			}
			//CS:S FIX v2 FIX HEALTH (WASN'T FIXED IN CS:GO TOO)
			//else if(player.GetHealth() >= 600 && player.GetTeam() == 2)
			if(player.GetHealth() >= 600 && player.GetTeam() == 2)
			{
				if(pl.setPerks == false)
				{
					if(pl.tskin != null)
						player.SetModel(pl.tskin);

					local alldone = true;

					if(pl.slow == false)
						EntFireByHandle(SpeedMod, "ModifySpeed", (pl.ReturnSpeed()).tostring(), 0.1, player, player);
					else
						alldone = false;

					if(pl.perkchameleon_lvl > 0)
					{
						player.__KeyValueFromInt("rendermode", 1);
						player.__KeyValueFromInt("renderamt", (255 - (pl.perkchameleon_lvl * perkchameleon_chameleonperlvl)));
					}

					local hp = MaxHPZombie + pl.perkhp_zm_lvl * perkhp_zm_hpperlvl;
					if (CLASSIC_MOD)
					{
					...
					{
						local waffel_car = Entities.FindByName(null, "waffel_controller");
						waffel_car.GetScriptScope().DestroyCar(player);
						EntFire("kojima_chest_" + pl.userid, "Kill", "", 0, null);
						pl.setPerks = true;
						pl.invalid = false;
					}
				}
				...
				else if(pl.perkhp_zm_lvl > 0)
				{
					MaxHP += pl.perkhp_zm_lvl * perkhp_zm_hpperlvl;
				}
	...
	OnGameEvent_player_jump = function(params)
	{
		...
		if(pl != null)
		{
			...
			{
				//CS:S FIX
				//Jump save if the first failed i don't think it's possible
				//local handle = pl.handle;
				//handle.SetVelocity(Vector(handle.GetVelocity().x,handle.GetVelocity().y,0));
				NetProps.SetPropBool(pl.handle, "pl.deadflag", true);
			}
		}
	}
	OnGameEvent_player_death = function(params)
	{
		local victim_userid = params.userid;
		local attacker_userid = params.attacker;
		...
		try
		{
			EntFireByHandle(victim_userid, "RunScriptCode", "NetProps.SetPropBool(victim_userid, \"pl.deadflag\", false);", 7.5, null, null); //CS:S FIX JUMP
		}
        catch(error)
        {
			return;
	...
	{
		local userid = params.userid
		local steamid = params.networkid
		local pl = GetPlayerClassByUserID(userid);
		if(pl != null)
		{
			if(pl.steamid == steamid && params.bot != false && pl.steamid != "Reconnect PLS")
			{
	...
	{
		EntFireByHandle(self, "CallScriptFunction", "CleanEntities", 4, null, null);
	}
	//CS:S fix v2 player_say moved here cause of getting called twice
	OnGameEvent_player_say = function(params)
	//try //CS:S FIX try re enabled cause of console spam
	{
		local userid = params.userid;
		...
		else if(Command_pref + "help" == args[0])
		{
			if(CHAT_CAN)
			{
				CHAT_CAN = false;
				CSS_Help();
				//NEW function to fix delay since saycommands doesn't have self
				function CSS_FIX_COOLDOWN()
				{
					CHAT_CAN = true;
				}
				EntFireByHandle(self, "CallScriptFunction", "CSS_FIX_COOLDOWN", CHAT_HELP_CD, null, null);
			}
		}
		...
		{
			CSS_ToggleWaffel(userid);
		}

		else if(Command_pref + "waffelcar" == args[0])
		{
			CSS_WaffelCar(userid);
		}

		if(access > 0)
		{
			//CS:S FIX DISABLED SINCE CS:S DOESN'T HAS KNIFES
			...
			if(access > 1)
			{
				...
				{
					CSS_MapTarget(userid,
					(args.len() > 1) ? args[1] : "");
				}

				else if(Command_pref + "admin" == args[0])
				{
					CSS_TeleportAdminRoom(userid,
					(args.len() > 1) ? args[1] : "");
				}
			}
		}
...
function PrintPerksAll()
{
	local timer = 0.0;
	local delay = 3.5;
	EntFireByHandle(self, "CallScriptFunction", "PrintPerksInfo", timer, null, null);
	EntFireByHandle(self, "CallScriptFunction", "PrintPerksItem", timer += delay, null, null);
	EntFireByHandle(self, "CallScriptFunction", "PrintPerksPerk", timer += delay, null, null);
	EntFireByHandle(self, "CallScriptFunction", "PrintPerksPerk_hm", timer += delay, null, null);
	EntFireByHandle(self, "CallScriptFunction", "PrintPerksPerk_zm", timer += delay, null, null);
}
...
function RotateStartPropEnd()
{
	...
	EntFireByHandle(self, "CallScriptFunction", "RotateStartPropEnd", 0.05, null, null);
}
...
function Trigger_CosmoBar()
{
	...
	EntFire("css_fix_spawn_classic*", "Kill", "", 40.00); //CS:S FIX removing text/sprites on spawn from perks (CS:GO didN't had it)
	EntFire("css_fix_classic", "Kill", "", 40.00); //CS:S FIX material classic
...
function Trigger_Rock()
{
	...
	//EntFire("explosion", "RunScriptCode", "CreateExplosion(Vector(-4620,-4268,1758),228,100)", 20.01 + timer); //CS:S FIX un needed make confusion the shortcut top explode 2x
	EntFire("explosion", "RunScriptCode", "CreateExplosion(Vector(-4718,-4333,2159),228,100)", 20.02 + timer);

	EntFire("music01", "CallScriptFunction", "GetMusicExplosion", 20.05 + timer);
...
function Mine_Door3()
{
	...
	if(Stage == 2)
	{
		...
		EntFire("Temp_Scorpion", "CallScriptFunction", "Camera", 36);
		EntFire("music01", "CallScriptFunction", "GetMusicBossFight", 36);
		...
		EntFire("Camera_old", "CallScriptFunction", "SetOverLay", 41.5);
		EntFire("Temp_Scorpion", "CallScriptFunction", "Start", 43);
	}
	if(Stage == 5)
	{
		local text;
...
function Bhop_Toggle(value = false, show = false)
{
	...
	if(show)
	{
		...
		text = "BHOP" + ((value) ? "\x0740FF3D Enable" : "\x07FE0000 Disable");
		ServerChat(Chat_pref + text);
	}
}
...
function Trigger_Cave_Boss_Start()
{
	...
	if(Stage == 1 || Stage == 4)
	{
		...
		EntFire("Temp_Gi_Nattak", "CallScriptFunction", "Init", 5.01);
	}
	if(Stage == 5)
	{
		EntFire("Temp_AirBuster", "ForceSpawn", "", 4.9);
		EntFire("Temp_AirBuster", "CallScriptFunction", "PreStart", 5);
	}
}
...
function Trigger_Cave_After_Boss()
{
	...
	EntFire("music01", "CallScriptFunction", "GetMusicAfterBoss", 0.00);
}
...
//Normal
{
	...
	function Trigger_Normal_Win()
	{
		...
		text = "NORMAL mode has been beaten. Unlocking HARD mode"
		ServerChat(Chat_pref + text);
...
//Hard
{
	...
	function Trigger_Hard_Win()
	{
		...
		text = "HARD mode has been beaten. Unlocking ZM mode"
		ServerChat(Chat_pref + text);
...
//ZM
{
	...
	function Trigger_ZM_End_Win()
	{
		...
		text = "ZM mode has been beaten. Unlocking EXTREME mode"
		ServerChat(Chat_pref + text);
		...
		local g_round = Entities.FindByName(null, "round_end");
		EntFireByHandle(g_round, "FireWinCondition", "7", 1.8, null, null);

		Show_Credits_Passed();

		EntFire("music01", "RunScriptCode", "SetMusic(Sound_Win);", 0.00);
		EntFire("Nuke_fade", "Fade", "", 0.00);
		EntFire("zamok_ct", "CallScriptFunction", "Stop", 0);
...
//EXTREME
{
	function Trigger_New_End()
	{
		...
		if(Stage == 4)
		{
			...
			EntFire("Temp_Extreme", "CallScriptFunction", "Init", 0.01);
		...
		{
			EntFire("Temp_Reno", "CallScriptFunction", "Start", 0);
			EntFire("Map_TD", "AddOutput", "angles 0 180 0", 5);
...
function Trigger_New_End_Last()
{
	if(Stage == 4)
	{
		...
		//CS:S FIX i had to sink the new cam function
		//EntFire("Camera_old", "RunScriptCode", "SpawnCameras(Vector(-8225,-982,1878),Vector(0,45,0),0,Vector(-8695,-982,1878),Vector(0,45,0),1,2.4,200)", 8); //CS:GO 0+1+2.4
		//EntFire("Camera_old", "RunScriptCode", "SpawnCameras(Vector(-10413,-928,1833),Vector(0,0,0),3,Vector(-10413,-928,1897),Vector(0,0,0),0,1)", 3.4 + 8); //CS:GO 3+1
		...
		EntFire("Camera_old", "RunScriptCode", "SpawnCameras(Vector(-8225,-982,1878),Vector(0,45,0),0,Vector(-8695,-982,1878),Vector(0,45,0),1,2.4,200)", 8);
		EntFire("Camera_old", "RunScriptCode", "SpawnCameras(Vector(-10413,-928,1833),Vector(0,0,0),3,Vector(-10413,-928,1833),Vector(0,0,0),0,0.6)", 3.4 + 8); //3.6s delay
		EntFire("Camera_old", "RunScriptCode", "SpawnCameras(Vector(-10413,-928,1833),Vector(0,0,0),0,Vector(-10413,-928,1897),Vector(0,0,0),0,0.4,200)", 6.6 + 8); //0.4s delay
		EntFire("Camera_old", "RunScriptCode", "SpawnCameras(Vector(-10413,-928,1897),Vector(0,0,0),0,Vector(-10413,-928,1897),Vector(0,0,0),0,0.4)", 7 + 8); //0.4s delay
		EntFire("Camera_old", "RunScriptCode", "SpawnCameras(Vector(-10413,-928,1897),Vector(0,0,0),0,Vector(-10839,-928,1897),Vector(0,0,0),1,2.7,200)", 7.4 + 8);

		EntFire("Camera_old", "CallScriptFunction", "SetOverLay", 11.1 + 8);
...
function Trigger_Win()
	{
		...
		if (Stage == 4)
		{
			text = "EXTREME mode has been beaten. Unlocking INFERNO mode"
			ServerChat(Chat_pref + text);
		...
		{
			text = "Inferno mode has been beaten. Thanks for test Cosmo v6"
			ServerChat(Chat_pref + text);
...
function ShowCreditsText(text, delay = 0.00)
{
	...
	EntFireByHandle(CreditsText, "AddOutput", "message " + text, delay, null, null);
...
function RotateSkin()
{
	...
	EntFireByHandle(self, "CallScriptFunction", "RotateSkin", 0.05, null, null);
}
...
function ShowPlayerText(handle, text)
{
	...
	if(PlayerText == null || PlayerText != null && !PlayerText.IsValid())
		return;
	//CS:S FIX set text changed to addoutput
	EntFireByHandle(PlayerText, "AddOutput", "message " + text, 0, handle, handle);
...
function Trigger_Invalid()
{
    ...
    if(car_class == null)
    {
		//Allow_Waffel is in saycommands.nut and on adminroom
...
::Mes_Warmup <- " \x07FE0000 *** \x07E1AE39 Warmup \x07FE0000 ***";
::RED <- "\x07FE0000"
::GREEN <- "\x0740FF3D"
::YELLOW <- "\x07ECE37A"
::IDLE <- "IDLE";
...
::TEMP_PHOENIX <- "item_temp_phoenix"; //CS:S FIX COLOR (CS:GO item_text_phoenix color1 7 50 248) set to (170 185 255) cause transparency issue
::TEMP_WAFFEL <- "waffel_controller_spawner";


ServerSettings <-[
//"ammo_50AE_max 6000", //CS:S DISABLED
//"ammo_762mm_max 6000", //CS:S DISABLED
//"ammo_556mm_box_max 6000", //CS:S DISABLED
//"ammo_556mm_max 6000", //CS:S DISABLED
//"ammo_338mag_max 6000", //CS:S DISABLED
//"ammo_9mm_max 6000", //CS:S DISABLED
//"ammo_buckshot_max 6000", //CS:S DISABLED
//"ammo_45acp_max 6000", //CS:S DISABLED
//"ammo_357sig_max 6000", //CS:S DISABLED
//"ammo_57mm_max 6000", //CS:S DISABLED
...
//"ze_infect_time 40", CS:S DOESN'T HAVE IT

"zr_ztele_zombie 1",
"zr_ztele_max_zombie 5",
"zr_ztele_max_human 1",

//"sm_rst_m249 0", CS:S DOESN'T HAVE IT
//"sm_rst_negev 0", CS:S DOESN'T HAVE IT
"zr_unrestrict m249" //CS:S FIX
...
function StartCD(cooldown = 0)
{
	...
	if(item.cd > 0)
	{
		EntFireByHandle(self, "CallScriptFunction", "StartCD", 1, caller, caller);
	}
	else
	{
		if(item.type == 1)
...
function BeatStage() //До установки
{
	...
	local text = "Map Start " + ((Map_Start_Time / 60) % 60).tointeger() + " minutes " + (Map_Start_Time % 60).tointeger() + " seconds";
	ServerChat(Chat_pref + text)
	...
	if (Stage_Beat_Normal_Time != null)
	{
		...
		local text = "Normal mode beaten in " + ((Time / 60) % 60).tointeger() + " minutes " + (Time % 60).tointeger() + " seconds, " + Stage_Beat_Normal_Count + " tries";
		ServerChat(Chat_pref + text)
	}
	...
	if (Stage_Beat_Hard_Time != null)
	{
		...
		local szTime = "";
		local szSumTime = "(Sum " + ((SumTime / 60) % 60).tointeger() + " minutes " + (SumTime % 60).tointeger() + " seconds)";

		if (Stage_Beat_Normal_Time != null)
		{
			SumTime = Stage_Beat_Hard_Time - Stage_Beat_Normal_Time;
			szTime = "Over " +((SumTime / 60) % 60).tointeger() + " minutes " + (SumTime % 60).tointeger() + " seconds";
		}

		local text = "Hard mode was been beaten " + szTime + "" + szSumTime + ", and " + Stage_Beat_Hard_Count + " attempts";
		ServerChat(Chat_pref + text)
	}
	...
	if (Stage_Beat_ZM_Time != null)
	{
		...
		local text = "ZM mode was been beaten " + szTime + "" + szSumTime + ", and " + Stage_Beat_ZM_Count + " attempts";
		ServerChat(Chat_pref + text)
	}
	...
	if (Stage_Beat_Extreme_Time != null)
	{
		...
		local text = "Extreme mode was been beaten " + szTime + "" + szSumTime + ", and " + Stage_Beat_Extreme_Count + " attempts";
		ServerChat(Chat_pref + text)
	}
	...
	if (Stage_Beat_Inferno_Time != null)
	{
		...
		local text = "Inferno mode was been beaten " + szTime + "" + szSumTime + ", and " + Stage_Beat_Inferno_Count + " attempts";
		ServerChat(Chat_pref + text)
	}
}
...
//////Skin/////
///////////////
//::CT_MODEL <- "models/player/custom_player/legacy/ctm_sas_variantf.mdl"; //CS:S FIX
