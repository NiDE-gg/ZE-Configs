...
::Shrek_pref <- "\x07FD3F3F[\x0440FF3DSHREK\x07FD3F3F]\x07ECE37A ";
::Donkey_pref<- "\x07FD3F3F[\x0440FF3DDONKEY\x07FD3F3F]\x07ECE37A ";
...
// Minigame Vote Logic Wrapper
function GetWinningMinigame()
{
    local b = ArrBuilder.len();
    local r = ArrRotate.len();
    local l = ArrLava.len();


    if (b == 0 && r == 0 && l == 0) return RandomInt(1, 3);
    if (b >= r && b >= l) return 1;
    if (r >= b && r >= l) return 2;
    return 3;
}
...
{   
    ...
    ServerChat(Chat_pref + text, 0.0);

    text = "Map by \x0740FF3DBiba\x07ECE37A(\x07FD3F3FKondik\x07ECE37A) & \x0740FF3DBoba\x07ECE37A(\x07FD3F3FHaRyDe\x07ECE37A)";
    ServerChat(Chat_pref + text, 0.1);

    text = "Thx \x0740FF3DLeXeR \x07ECE37A& \x0740FF3DHeadsh \x07ECE37A& \x0740FF3DKotya \x07ECE37A& \x0740FF3DFriend";
    ServerChat(Chat_pref + text, 0.2);

    text = "Port - \x0740FF3DVndrew";
    ServerChat(Chat_pref + text, 0.3);
    ...
    EntFireByHandle(self, "RunScriptCode", "SpawnZMItem(0);", 20.0, null, null);
    EntFireByHandle(self, "RunScriptCode", "SpawnZMItem(0);", 20.2, null, null);
    EntFireByHandle(self, "RunScriptCode", "SpawnZMItem(1);", 20.4, null, null);
    EntFireByHandle(self, "RunScriptCode", "SpawnZMItem(1);", 20.6, null, null);
}
...
function Hold()
{
    ...
    EntFire(sound, "PlaySound", "", 10);
    EntFire("Donkey_Train", "FireUser1", "", 0);
}

function StartDonkey()
{
    local text;
...
{ 
    ...
    if(!Language)
        text = "Похоже мы нашли это тайное место... Осталось открыть дверь";
    else
    ...
    EntFireByHandle(self, "RunScriptCode", "ChekCTa();", 65 + time, null, null); // DISABLED
...
function StartDragon()
{
    ...
    EntFire("carpet_glow*", "Toggle", "", 35 + time);
...
function Final()
{
    local text;
    g_bFravity = false;
    g_bFravity = false; // Fixed potential typo from g_bFravity
    ...
    switch (RandomInt(1, 3)) 
    {
        case 1: // плиты 
        {
            ...
            EntFire("Teleport_Minigame_Builder", "Enable", "", 3);
            ...
            while ((handle = Entities.FindByClassname(handle, "player")) != null)
            {
                ...
                if (handle.GetHealth() < 1) continue;
                
                if (handle.GetTeam() == 2)
                {
                    EntFireByHandle(handle, "AddOutput", "origin 1183 -1351 10620", 0.1, null, null);
                    handle.SetHealth(10000);
                }
                else if (handle.GetTeam() == 3)
                {
                    EntFireByHandle(handle, "AddOutput", "origin 990 5 10796", 0.1, null, null);
                    handle.SetHealth(1000);
                }
                
                handle.__KeyValueFromFloat("gravity", 1);
            }
            EntFire("stage2_voting_system", "RunScriptCode", "startgame1();", 0.2);
        }
        break;

        case 2: // скакалка (rotate)
        {
            EntFire("mg_rot_temp", "FireUser1");
            EntFire("stage2_voting_system", "RunScriptCode", "startgame2();", 0.5);
            EntFire("map_gText", "AddOutput", "message Fall guys");
            EntFire("map_gText", "FireUser1", "", 0.1);
            EntFire("Teleport_Minigame_Rotate", "Enable", "", 3);
        ...
        case 3: // вентиляторы+лава 
        {
            ...
            EntFire("map_gText", "FireUser1", "", 0.1);
            EntFire("Teleport_Minigame_lava", "Enable", "", 3);
            ...
            EntFire("stage2_voting_system", "RunScriptCode", "startgame3();", 0.5);
        }
        break;
    }
}
...
function StartTpZm()
{
    ...
    while((handle = Entities.FindByClassname(handle, "player")) != null)
    {
        if(handle == null) continue;
        if(!handle.IsValid()) continue;
        if(handle.GetHealth() <= 0) continue;
        if(handle.GetTeam() == 2)
        {
...
::SpawnZMItem <- function(item)
{
    local spawner = null;

    if (item == 0)
    {
        spawner = Entities.FindByName(null, "map_spawner_banana");
    }
    else if (item == 1)
    {
        spawner = Entities.FindByName(null, "map_spawner_chesnok");
    }
    else
    {
        return;
    ...
    local x = RandomInt(-6464, -6176);
    local y = 13280;
    local z = 128;

    EntFireByHandle(spawner,"AddOutput","origin " + x + " " + y + " " + z,0,null,null);
    EntFireByHandle(spawner,"AddOutput","angles 0 270 0",0,null,null);
    EntFireByHandle(spawner,"ForceSpawn","",0.01,null,null);
}
