...
item_use_snd <- "physics/glass/glass_impact_bullet3.wav"
item_hit_snd <- "physics/glass/glass_strain2.wav"
...
function ItemPickup(activator) {
	...
	if (first_pickup)
		MapPrint("A human has picked up an \x0722ffffIce Crystal")

	first_pickup = false
	EntFire("item_whitegate_icecrystal_text", "Display", null, 0.0, item_holder)
}
...
function ItemHolderTick(player, buttons, buttons_changed, buttons_pressed, buttons_released) {
	...
	if (heal_wave_cd_left <= 0 && white_bossfight) {
		heal_wave_cd_left = heal_wave_cd

		RunWithDelay(@() DispatchParticleEffect("item_icecrystal_healwave", self.GetOrigin() + Vector(0, 0, 36), QAngle().Forward()), 0.0)
		for (local ent; ent = Entities.FindByClassnameWithin(ent, "player", self.GetOrigin(), 256);) {
			...
			local new_health = ent.GetHealth() + 5
			ent.SetHealth(min(new_health, ent.GetMaxHealth()))
		}
	}
	else if (white_bossfight && heal_wave_cd_left > 0) heal_wave_cd_left -= FrameTime()
...
function ItemUse() {
	...
	local projectile = CreateEntity("info_particle_system", {
		...
		start_active = true
		effect_name = "item_icecrystal_projectile"
	})
	...
	p_scope.DestroyProjectile <- function() {
		EmitSoundEx({
			sound_name = "physics/glass/glass_strain2.wav"
			channel = 6
			sound_level = 80
			origin = self.GetOrigin()
		})
		DispatchParticleEffect("item_icecrystal_impact", self.GetCenter(), QAngle().Forward())
		...
		local field = CreateEntity("info_particle_system", {
			...
			angles = QAngle()
			start_active = true
			effect_name = "item_icecrystal_field"
		})
		...
		scope.Think <- function() {
			for (local ent; ent = Entities.FindByClassnameWithin(ent, "player", self.GetOrigin(), 256);) {
				...
				SetPlayerSpeed(ent, 0.0)
				frozen_players[ent] <- true
			}

			if (timer <= 0) {
				foreach(player, data in frozen_players) {
	...
	h_scope.Trigger <- function(activator) {
		if (activator.GetTeam() == TEAM_ZOMBIES) {
...
function EndCooldown() {
	item_ready = true
	ClientPrint(item_holder, 3, "\x07fcd9f1[Item] \x0722ffffIce Crystal \x07defafcis ready!")
}